    ...
    public static function buildAppointmentsForCalendar( Lib\Query $query, $display_tz )
    {
        ...
        foreach( $staff_id as $staff_id_hwe )
        {
           ...
           $number_of_persons = $staff_id_hwe['total_number_of_persons'];
            ...
            FROM ".$wpdb->prefix."bookly_customers AS c WHERE id='".$staff_auto_id."'",ARRAY_A);
        ...
        foreach ( $appointments as $key => $appointment ) {
           ...
           else if($appointment['customers'][0]['status'] == 'pending')
           {
            ...
            $tablebookly=$wpdb->prefix."bookly_services";
            $selectappointments="select * from $tablebookly where id='".$appointment['service_id_hwe']."'";
            $resultsappointments=$wpdb->get_results($selectappointments,ARRAY_A);
            foreach($resultsappointments as $resultsappointments_hwe)
            {
             $service_title='';
                $service_title =  $resultsappointments_hwe['title'];
                $appointment['service_name'] = $service_title;
            }
 
            $codes['service_name']     = $appointment['service_name'] ? esc_html( $appointment['service_name'] ) : __( $service_title, 'bookly' );
