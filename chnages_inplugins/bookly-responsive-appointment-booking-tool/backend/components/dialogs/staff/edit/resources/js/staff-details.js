    ...
    var Details = function ($container, options) {
        ...
        $staff_wp_user.on('change', function () {
            if (this.value && this.value !== 'create') {
                ...
                $staff_gender.val($staff_wp_user.find(':selected').text());
                $staff_zip_code.val($staff_wp_user.find(':selected').val()); 
            }
        });
...
//                 var datahwe = JSON.stringify(data);
// console.log(datahwe);
                data.gender = genderhwe;
                data.zip_code = zipcodehwe;
