...
//mohit
global $wpdb;
$results = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."bookly_staff WHERE id ='".$staff->getId()."'", ARRAY_A );
foreach($results as $new_field)
{

   $male =$new_field['gender'];
   $zip_code =$new_field['zip_code'];
...
jQuery(document).ready(function(){
    ...
    jQuery("#bookly-zip_code").val(zip_code);
    jQuery("#bookly-zip_code").attr("value",zip_code);
    jQuery("#bookly-gender").val(male).change();
