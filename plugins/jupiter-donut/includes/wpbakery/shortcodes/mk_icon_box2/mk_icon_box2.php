...
if ( 'icon' === $icon_type ) {
	$icon_size = 'inherit' === $icon_size ? 16 : $icon_size ;
...
$output .= '<div id="mk-icon-box-' . $id . '" class="mk-box-icon-2 ' . $el_class . ' jupiter-donut-' . $visibility . ' box-align-' . $align . ' ' . get_viewport_animation_class( $animation ) . '">';
if ( 'icon' == $icon_type ) {
	$app_styles = '
	...
	$output .= ! empty( $read_more_url ) ? '<a href="' . $read_more_url . '" target="' . $link_target . '">' : '';
	$output .= '        <img src="' . mk_get_wpml_media( $icon_image ) . '" alt="' . $title . '" />';
	$output .= ! empty( $read_more_url ) ? '</a>' : '';
	$output .= '    </div>';
}
