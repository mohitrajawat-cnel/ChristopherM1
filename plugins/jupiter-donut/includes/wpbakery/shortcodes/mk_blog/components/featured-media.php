...
switch ( $view_params['post_type'] ) {
	case 'image':
		if ( ! Mk_Image_Resize::is_default_thumb( $image_src_array[0] ) ) {
			...
			$lighbox = isset( $mk_options['single_disable_featured_image_lightbox'] ) ? $mk_options['single_disable_featured_image_lightbox'] : 'false';

			if ( 'true' === $lighbox ) {
				$image_html = sprintf(
					'<div class="featured-image"><a class="full-cover-link %1$s" title="%2$s" href="%3$s">&nbsp;</a>',
					esc_attr( $image_permalink_class ),
					the_title_attribute(
						array(
	...
	case 'video':
		...
		if ( 'youtube' == $video_site ) {
			echo '<div class="mk-video-wrapper"><div class="mk-video-container"><iframe src="//www.' . mk_get_third_party_domain_name( 'youtube' ) . '/embed/' . esc_attr( $video_id ) . '?showinfo=0&amp;theme=light&amp;color=white&amp;rel=0" frameborder="0" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div></div>';
		}
