...
if ( ($global_lazyload == 'true' && $disable_lazyload == 'false') || ($global_lazyload == 'false' && $lazyload == 'true') ) {
     $lazylaod_class = 'mk-image-lazyload';
}
...
$output .= '<div ' . $parallax_scroll . ' class="mk-image mk-image-' . $id . ' jupiter-donut-' . $visibility . ' ' . $lightbox_enabled . ' ' . $lazylaod_class . ' align-' . $align . ' ' . get_viewport_animation_class( $animation ) . $frame_style . '-frame ' . $caption_location . ' ' . $el_class . '" style="margin-bottom:' . $margin_bottom . 'px">';
...
if ( 'true' == $lightbox ) {
	...
	$output .= '<a href="' . $src_lightbox . '" alt="' . $image_alt . '" data-fancybox="image-shortcode-' . $group . '" title="' . esc_attr( $title ) . '" class="mk-lightbox ' . $group . ' mk-image-lightbox">' . Mk_SVG_Icons::get_svg_icon_by_class_name( false, 'mk-jupiter-icon-plus-circle' ) . '</a>';
}
