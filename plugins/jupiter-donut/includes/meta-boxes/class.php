...
class mkMetaboxesGenerator {
	...
	function upload( $value ) {
		...
		$field .= '<input class="mk-upload-url" type="text" id="' . $value['id'] . '" name="' . $value['id'] . '" size="50"  value="' . $value['default'] . '" /><a class="option-upload-button secondary-button thickbox" id="' . $value['id'] . '_button" href="#">' . __( 'Upload', 'jupiter-donut' ) . '</a>';
		if ( ! empty( $value['width'] ) ) {
			$field .= '<div class="mk-upload-width-wrap">
				<label>' . __( 'Width', 'jupiter-donut' ) . '</label>
	...
	function header_styles( $value ) {
		...
		$field .= '<div class="label">' . __( 'Align', 'jupiter-donut' ) . '</div>';
		...
		$field .= '<div class="label">' . __( 'Toolbar', 'jupiter-donut' ) . '</div>';
	...
	function select( $value ) {
		...
		$field .= '<option value="">' . __( 'Select Option', 'jupiter-donut' ) . '</div>';
		foreach ( $value['options'] as $key => $option ) {
	...
	function visual_selector( $value ) {
		...
		foreach ( $value['options'] as $key => $option ) {
			$field .= '<a href="#" rel="' . $key . '"><img  src="' . JUPITER_DONUT_ASSETS_URL . '/img/' . $option . '.png" /></a>';
		}
		$field .= '<input type="hidden" value="' . $value['default'] . '" name="' . $value['id'] . '" id="' . $value['id'] . '"/>';
		$field .= '</div>';
	...
	function superlink( $value ) {
		...
		$field .= '<option value="">' . __( 'Select Linking method', 'jupiter-donut' ) . '</option>';
		...
		$field .= '<op$field .=tion value="">' . __( 'Select Page', 'jupiter-donut' ) . '</option>';
		...
		$field .= '<option value="">' . __( 'Select Portfolio', 'jupiter-donut' ) . '</option>';
		...
		$field .= '<option value="">' . __( 'Select Category', 'jupiter-donut' ) . '</option>';
		...
		$field .= '<option value="">' . __( 'Select Post', 'jupiter-donut' ) . '</option>';
						...
						<div id="mk-bg-edit-panel" class="mk-bg-edit-panel">
							...
							<div style="border-bottom:1px solid #ccc;">
								<div class="mk-bg-edit-right">
									<div class="mk-bg-edit-option mk-bg-edit-bg-color"> <span class="mk-bg-edit-label"><?php _e( 'Background color', 'jupiter-donut' ); ?></span>
										<div class="bg-edit-panel-color">
										  <select class="mk-select bg-panel-color-style" name="bg_panel_color_style" id="bg_panel_color_style">
											<option value="single"><?php _e( 'Single Color', 'jupiter-donut' ); ?></option>
											<option value="gradient"><?php _e( 'Gradient', 'jupiter-donut' ); ?></option>
										  </select>
										</div>
										<div class="clearboth"></div>
										<div class="bg-edit-panel-color">
										  <span class="mk-bg-edit-label panel-gradient-element"><?php _e( 'From', 'jupiter-donut' ); ?></span>
										...
										<div class="bg-edit-panel-color panel-gradient-element">
										  <span class="mk-bg-edit-label"><?php _e( 'To', 'jupiter-donut' ); ?></span>
										  <div class="color-picker-holder"><input name="bg_panel_color_2" id="bg_panel_color_2" size="8" class="color-picker" value="" /></div>
										</div>
										<div class="clearboth"></div>
										<div class="bg-edit-panel-color panel-gradient-element">
										  <span class="mk-bg-edit-label"><?php _e( 'Style', 'jupiter-donut' ); ?></span>
										  <select class="mk-select" name="grandient_color_style" id="grandient_color_style">
											<option value="linear"><?php _e( 'Linear', 'jupiter-donut' ); ?></option>
											<option value="radial"><?php _e( 'Radial', 'jupiter-donut' ); ?></option>
										  </select>
										</div>

										<div class="bg-edit-panel-color panel-gradient-element panel-linear-gradient-el">
										  <span class="mk-bg-edit-label"><?php _e( 'Angle', 'jupiter-donut' ); ?></span>
										  <select name="grandient_color_angle" id="grandient_color_angle" class="mk-select">
											<option value="vertical"><?php _e( 'Vertical ↓', 'jupiter-donut' ); ?></option>
											<option value="horizontal"><?php _e( 'Horizontal →', 'jupiter-donut' ); ?></option>
											<option value="diagonal_left_bottom"><?php _e( 'Diagonal ↘', 'jupiter-donut' ); ?></option>
											<option value="diagonal_left_top"><?php _e( 'Diagonal ↗', 'jupiter-donut' ); ?></option>
										  </select>
										</div>
										...
										<div class="bg-background-type-panes">
											...
											<div class="bg-background-type-pane bg-edit-panel-upload">
												<div class="upload-option">
													...
													<?php _e( 'Upload a new custom image', 'jupiter-donut' ); ?>
													</span>
													<div class="mk-upload-bg-wrapper">
														<input class="mk-upload-url" type="text" id="bg_panel_upload" name="bg_panel_upload" size="40"  value="" />
														<a class="option-upload-button secondary-button thickbox" id="bg_panel_upload_button" href="#"><?php _e( 'Upload', 'jupiter-donut' ); ?></a>
								...
								<div class="mk-bg-edit-left">
									<div class="mk-bg-edit-option mk-bg-edit-option-repeat"> <span class="mk-bg-edit-label">
										<?php _e( 'Background Repeat', 'jupiter-donut' ); ?>
										</span>
									...
									<div class="mk-bg-edit-option mk-bg-edit-option-attachment"> <span class="mk-bg-edit-label">
										<?php _e( 'Background Attachment', 'jupiter-donut' ); ?>
										</span>
									...
									<div class="mk-bg-edit-option mk-bg-edit-option-position"> <span class="mk-bg-edit-label"><?php _e( 'Background Position', 'jupiter-donut' ); ?></span>
									...
									<div class="mk-bg-edit-option mk-bg-edit-option-stretch">
										<span class="mk-bg-edit-label"><?php _e( 'Enable Parallax Effect', 'jupiter-donut' ); ?></span>
									...
									<div class="mk-bg-edit-option mk-bg-edit-option-stretch">
										<span class="mk-bg-edit-label"><?php _e( 'Cover whole background', 'jupiter-donut' ); ?></span>
										<span class="mk-toggle-button">
											<span class="toggle-handle"></span>
			...
			if ( $attachments ) {
				foreach ( $attachments as $attachment_id ) {
					echo '<li class="image attachment details" data-attachment_id="' . $attachment_id . '"><div class="attachment-preview"><div class="thumbnail">
                            ...
                            <a href="#" class="delete check" title="' . __( 'Remove image', 'jupiter-donut' ) . '"><div class="media-modal-icon"></div></a>
...
<script type="text/javascript">
		...
		var $image_gallery_ids = $('#<?php echo esc_attr( $value['id'] ); ?>');
			...
			jQuery('.add_gallery_images').on( 'click', 'a', function( event ) {
				...
				image_gallery_frame = wp.media.frames.downloadable_file = wp.media({
					...
					title: '<?php esc_html_e( 'Add Images to Gallery', 'jupiter-donut' ); ?>',
					button: {
						text: '<?php esc_html_e( 'Add to gallery', 'jupiter-donut' ); ?>',
					},
					multiple: true
				});
							 ...
							 $gallery_images.append('\
								<li class="image attachment details" data-attachment_id="' + attachment.id + '">\
									<div class="attachment-preview">\
									   ...
									   <a href="#" class="delete check" title="<?php esc_html_e( 'Remove image', 'jupiter-donut' ); ?>"><div class="media-modal-icon"></div></a>\
									</div>\
								</li>');
    ...
    */
	function get_select_target_options( $type ) {
		...
		switch ( $type ) {
			case 'page':
				$entries = get_pages( "title_li=&orderby=name&number={$post_count}" );
			...
			case 'post':
				$entries = get_posts( "orderby=title&numberposts={$post_count}&order=ASC&suppress_filters=0" );
				foreach ( $entries as $key => $entry ) {
					$options[ $entry->ID ] = $entry->post_title;
				}
				break;
			...
			case 'portfolio':
				$entries = get_posts( "post_type=portfolio&orderby=title&numberposts={$post_count}&order=ASC&suppress_filters=0" );
				foreach ( $entries as $key => $entry ) {
					$options[ $entry->ID ] = $entry->post_title;
				}
				break;
			...
			case 'flexslider':
				$entries = get_posts( "post_type=slideshow&orderby=title&numberposts={$post_count}&order=ASC&suppress_filters=0" );
				foreach ( $entries as $key => $entry ) {
					$options[ $entry->ID ] = $entry->post_title;
				}
				break;
			...
			case 'banner_builder':
				$entries = get_posts( "post_type=banner_builder&orderby=title&numberposts={$post_count}&order=ASC&suppress_filters=0" );
				foreach ( $entries as $key => $entry ) {
					$options[ $entry->ID ] = $entry->post_title;
				}
				break;
			...
			case 'edge':
				$entries = get_posts( "post_type=edge&orderby=title&numberposts={$post_count}&order=ASC&suppress_filters=0" );
				foreach ( $entries as $key => $entry ) {
					$options[ $entry->ID ] = $entry->post_title;
				}
				break;
...
if ( ! function_exists( 'mk_load_metaboxes' ) ) {
	function mk_load_metaboxes() {
		...
		$metabox_path = jupiter_donut()->plugin_dir() . '/includes/meta-boxes/fields/';
