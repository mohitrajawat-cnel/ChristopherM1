...
class Mk_Image_Resize {
	...
	public static function resize_by_url( $image_url, $width, $height, $crop = true, $dummy = true ) {
		...
		$image_url = mk_get_wpml_media( $image_url );
	...
	public static function resize_by_url_adaptive( $image_url, $width, $height, $crop = true, $dummy = true ) {
		...
		$responsive = jupiter_donut_get_option( 'responsive_images' );
		$retina = jupiter_donut_get_option( 'retina_images' );
	...
	public static function get_native_image_size_dimensions( $image_size, $src_array ) {
		...
		if ( empty( $src_array[1] ) ) {
			$src_array[1] = 0;
		}

		if ( empty( $src_array[2] ) ) {
			$src_array[2] = 0;
		}
	...
	static public function generate_dummy_image( $width = '', $height = '', $transparent = null ) {
		...
		$thumbnail_number = $transparent ? 'transparent' : $thumbnail_number;
				...
				$thumb_url = JUPITER_DONUT_ASSETS_URL . '/img/dummy-' . $thumbnail_number . '.png';
	...
	public static function get_bg_res_set( $landscape_url = false, $portrait_url = false, $enable_responsive = true ) {
		...
		$landscape_url = mk_get_wpml_media( $landscape_url );
		$portrait_url = mk_get_wpml_media( $portrait_url );
		...
		if ( $responsive == 'false' && ! empty( $original_image_url ) ) {
			$html_output .= 'style=background-image:url(\'' . $original_image_url . '\')';
		}
	...
	public function add_custom_image_sizes() {
		...
		$image_sizes = get_option( 'Jupiter_image_sizes' );
