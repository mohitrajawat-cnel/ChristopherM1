...
class Mk_SVG_Icons {
	...
	public function get_icons_db( $sort_alphabetically = true ) {
		...
		if ( is_null( $pagination_start ) || is_numeric( $pagination_start ) === false || $pagination_start < 0 ) {
			throw new Exception( __( 'Pagination start point is not valued correctly.' , 'jupiter-donut' ) );
			return false;
		}
		...
		if ( is_null( $pagination_count ) || is_numeric( $pagination_count ) === false || ($pagination_count < 1 && $pagination_count != -1) ) {
			throw new Exception( __( 'Pagination count point is not valued correctly.' , 'jupiter-donut' ) );
			return false;
		}

		// Check if icon's family is valued
		if ( $this->get_icon_family() == '' || $this->get_icon_family() == null ) {
			throw new Exception( __( 'Please choose an icon family.' , 'jupiter-donut' ) );
			return false;
		}
		...
		if ( $this->get_icon_family() !== 'all' && array_search( $this->get_icon_family() , self::$font_families ) === false ) {
			throw new Exception( __( 'Icon family that you choosed is not exist.' , 'jupiter-donut' ) );
			return false;
		}
		...
		foreach ( $icon_family as $key => $value ) {
			$map_json_address = '/icons/' . $value ;
			$map_json_uri = JUPITER_DONUT_ASSETS_URL . $map_json_address . '/map.json';
			$map_json_dir = JUPITER_DONUT_ASSETS_DIR . $map_json_address . '/map.json';

			if ( file_exists( $map_json_dir ) === false ) {
				throw new Exception( __( 'Font family icon datbase is not exist.' , 'jupiter-donut' ) );
				return false;
			}
		...
		if ( is_array( $icons_map ) === false || count( $icons_map ) < 1 ) {
				throw new Exception( __( 'Font family icon datbase have wrong structure , Call support.' , 'jupiter-donut' ) );
				return false;
		}
	...
	public function get_icons_list() {
		try {
			...
			if ( empty( $icon_family ) ) {
				throw new Exception( __( 'Icon family is not valued correctly.' , 'jupiter-donut' ) );
			}
			...
			$this->message( __( 'Successful' , 'jupiter-donut' ), true, $response );
			return true;
	...
	public function search_icon_by_name() {
		try {
			...
			if ( empty( $icon_name ) ) {
				throw new Exception( __( 'Icon name is not valued correctly.' , 'jupiter-donut' ) );
			}
			...
			if ( is_array( $matches_icon ) === false || count( $matches_icon ) < 1 ) {
				$this->message( __( 'Successful' , 'jupiter-donut' ), true, array() );
				return true;
			}
			...
			$this->message( __( 'Successful' , 'jupiter-donut' ), true, $matches_icon );
			return true;
	...
	public function get_class_name_by_unicode( $family, $unicode ) {
		...
		$file_path = '/icons/' . $family;
		$file_uri  = JUPITER_DONUT_ASSETS_URL . $file_path . '/map.json';
		$file_dir  = JUPITER_DONUT_ASSETS_DIR . $file_path . '/map.json';
	...
	static function get_svg_content( $family, $unicode ) {
		$file_path = '/icons/' . $family . '/svg/' . $unicode . '.svg';
		$file_uri = JUPITER_DONUT_ASSETS_URL . $file_path;
		$file_dir     = JUPITER_DONUT_ASSETS_DIR . $file_path;
